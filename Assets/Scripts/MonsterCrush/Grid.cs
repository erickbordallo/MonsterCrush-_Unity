...
public class Grid : MonoBehaviour
{
    ...
    public bool FillStep()
    {
        ...
        for (int x = 0; x < xDim; x++)
        {
            ...
            {
                Destroy(pieceBelow.gameObject);
                GameObject newPiece = Instantiate(piecePrefabDict[PieceType.NORMAL], GetWorldPosition(x, -1)*9.5f, Quaternion.identity);
    ...
    {
        Vector3 newTransform = new Vector3(transform.position.x - (xDim * 0.5f) + x
            , transform.position.y + (yDim * 0.5f) - y, 0.0f);
    ...
    {
        GameObject newPiece = Instantiate(piecePrefabDict[type], GetWorldPosition(x, y) * 9.5f, Quaternion.identity);
        newPiece.transform.parent = transform;
    ...
    public void ClearObstacles(int x, int y)
    {
        for (int adjacentX = x - 1; adjacentX <= x+1; adjacentX++)
        {
            ...
            {
                if(pieces[adjacentX, y].Type == PieceType.ASTEROID && pieces[adjacentX, y].IsClearable())
                {
        ...
        for (int adjacentY = y - 1; adjacentY <= y + 1; adjacentY++)
        {
            ...
            {
                if (pieces[x, adjacentY].Type == PieceType.ASTEROID && pieces[x, adjacentY].IsClearable())
                {
    ...
    private void InitializeValues()
    {
        ...
        if (levelType == "BreakingObstacle")
        {
            int numberOfObstacles = (int)jSonData.DataDictionary["NumberOfObstacles"];
            initialPieces = new PiecePosition[numberOfObstacles];

            int[] x = jSonData.DataDictionary["x"] as int[];
            int[] y = jSonData.DataDictionary["y"] as int[];

            for (int i = 0; i< numberOfObstacles; i++)
            {
                initialPieces[i].type = PieceType.ASTEROID;
                initialPieces[i].x = x[i];
                initialPieces[i].y = y[i];
            }
        }

        xDim = (int)jSonData.DataDictionary["xDim"];
        yDim = (int)jSonData.DataDictionary["yDim"];
    }
